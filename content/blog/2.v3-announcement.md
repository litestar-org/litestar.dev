...
mostly due to unforeseen happenstances, and the core maintainer team not being able to
spend as much time on the project as they would have liked. However, things have and are
still brewing, the project is growing, and significant progress has been made, so let's
...
- Type-based injection _alongside_ the existing name-based injection
...
`TypeDependency` alongside `NamedDependency`; The semantics remain the same, with the
...
class SomeMiddleware(ASGIMiddleware):
    async def handle(self, scope: Scope, send: Send, receive: Receive, next_app: ASGIApp) -> None:
        ...
        db_session =  await litestar_app.di_context.resolve(AsyncSession)
        ...
...
features it has: You can provide an object conforming to the stdlib's `logging.Logger`
interface or, alternatively, to structlog's logger interface, which can be toggled via a
...
applications are wired together, has been completely rewritten.

Litestar's routing generally works top-down:

- An application registers a router, a router registers a handler, etc.
...
- It had to (deep)copy things frequently, causing complicated workarounds, hampered
  startup performance and led to unexpected behaviour in some cases
...
[What's changed in 3.0](https://docs.litestar.dev/main/release-notes/whats-new-3.html) for an overview.
